...
By default, `logprob_scoring` is `TRUE`, which adds a `.confidence` column
alongside the originality scores. Set `logprob_scoring = FALSE` to disable it,
or rename the column with `confidence_col`:
...
The API works without authentication for moderate usage. To increase rate limits,
pass your key with the `api_key` argument (it is sent as the `X-API-KEY` header):
